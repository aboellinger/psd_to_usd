...
def convert(usd_stage, prim_path, image_path, psd_layer):
    ...
    usd_shade_preview_surface.CreateInput(
        "diffuseColor", Sdf.ValueTypeNames.Color3f
    ).ConnectToSource(usd_shade_uv_texture.ConnectableAPI(), "rgb")
    usd_shade_preview_surface.CreateInput("opacity", Sdf.ValueTypeNames.Float).Set(
        psd_layer.opacity / 255.0
    )
    usd_shade_preview_surface.CreateInput(
        "opacity", Sdf.ValueTypeNames.Color3f
    ).ConnectToSource(usd_shade_uv_texture.ConnectableAPI(), "a")
